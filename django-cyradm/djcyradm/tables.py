...
class DomainsTable(SelMixin, tables.Table):
    ...
    max_quota_per_account = tables.Column(verbose_name=_("Max quota per account"))
    max_aliases_per_account = tables.Column(verbose_name=_("Max aliases per account"))
    max_accounts_per_domain = tables.Column(verbose_name=_("Max accounts per domain"))
    max_external_aliases = tables.Column(verbose_name=_("Max external aliases"))
    external_aliases_used = tables.Column(verbose_name=_("External aliases used"), accessor='pk')

    class Meta:
        model = Domains
        attrs = {'class': 'table'}
        fields = ['selection', 'id', 'domain_name', 'max_quota_per_account', 'max_accounts_per_domain',
                  'max_aliases_per_account', 'max_external_aliases', 'external_aliases_used', 'is_alias_domain']
    ...
    def render_external_aliases_used(self, value):
        return str(VirtualDelivery.objects.filter(is_external_alias=True, alias_domain__id=value).count())
...
class MailUsersTable(SelMixin, tables.Table):
    ...
    domain = tables.LinkColumn(text=lambda record: record.domain.domain_name, viewname="domains-update",
                               verbose_name=_("Domain"), accessor="domain", args=[A('domain.id')])
    max_aliases = tables.Column(verbose_name=_("Max aliases"))
    max_aliases_in_domain = tables.Column(verbose_name=_("Max aliases (domain)"),
                                          accessor='domain.max_aliases_per_account')
    aliases_used = tables.Column(orderable=False, accessor='username', verbose_name=_("Aliases used"))
    quota = tables.Column(orderable=False, accessor='username', verbose_name=_('Quota'))
    admin_domains = tables.Column(orderable=False, verbose_name=_("Admindomains"))
    groups = tables.Column(orderable=False, verbose_name=_("Groups"), accessor="groups.all")
    is_active = tables.BooleanColumn(verbose_name=_("Is active"))
    actions = tables.TemplateColumn(template_name="djcyradm/action_column.html", verbose_name=_('Actions'))
    ...
    def render_aliases_used(self, value):
        return str(VirtualDelivery.objects.filter(is_forwarder=False, dest__username=value).count())

    class Meta:
        model = MailUsers
        exclude = ['password']
        fields = ['selection', 'id', 'username', 'domain', 'admin_domains', 'max_aliases', 'max_aliases_in_domain',
...
class VirtualDeliveryTable(SelMixin, tables.Table):
    ...
    is_external_alias = tables.BooleanColumn(verbose_name=_("Is external alias"))
