...
import djcyradm.overrides # noqa
...
class MySeleniumTests(StaticLiveServerTestCase):
    ...
    def test_make_aliases(self):
        ...
        self.make_alias_assert_success(
            alias="test",
            dest="myuser@example.com",
            alias_domain="example.com")
        ...
        self.make_alias(
            alias="test@tester1.dk",
            dest="myuser@example.com",
            alias_domain="example.com")
        self.make_alias_assert_error(
            error="tester1.dk is not the alias domain example.com")
        ...
        self.make_alias(
            ...
            alias_domain="example.com")
        self.make_alias_assert_error(
            error="A user has the wanted alias as username")
        ...
        self.assertEquals(Domains.objects.get(
            domain_name="example.com").max_aliases_per_account, 2)
        self.assertEqual(VirtualDelivery.objects.filter(
            full_dest="myuser@example.com").count(), 2)
        ...
        self.make_alias(
            ...
            alias_domain="example.com")
        self.make_alias_assert_error(
            error="No more aliases for this destination",
            idstr="id_dest",
            sel="select")
    ...
    def prep_add_ccount(self):
        ...
        if not MailUsers.objects.get_by_natural_key(
            "myuser@example.com").groups.filter(
                name="admins").exists() and\
                     not MailUsers.objects.get_by_natural_key(
                         "myuser@example.com").groups.filter(
    ...
    def test_addaccount_max_quota_to_big(self):
        ...
        if self.prep_add_ccount():
            ...
            self.make_alias_assert_error(
                idstr="id_quota",
                error="The max quota for domain example.com is " + str(
                    Domains.objects.filter(
                        domain_name="example.com")
                    .first().max_quota_per_account))
            return
    ...
    def test_add_account_max_aliases_to_big(self):
        ...
        if self.prep_add_ccount():
            ...
            self.make_alias_assert_error(
                ...
                error="The max number of aliases per account "
                      "for domain example.com is " + str(
                        Domains.objects.filter(
                            domain_name="example.com"
                            ).first().max_aliases_per_account))
            return
    ...
    def test_addaccount_alias_and_quota_to_big(self):
        ...
        if self.prep_add_ccount():
            ...
            self.make_alias_assert_error(
                idstr="id_max_aliases",
                error="The max number of aliases per account " +
                "for domain example.com is " +
                str(Domains.objects.filter(
                     domain_name="example.com")
                    .first().max_aliases_per_account))
            self.make_alias_assert_error(
                idstr="id_quota",
                error="The max quota for domain example.com is " +
                str(Domains.objects.filter(
                    domain_name="example.com").first().max_quota_per_account))
...
class SimpleTest(TransactionTestCase):
    ...
    def test_login(self):
        ...
        self.assertEquals(self.client.post('/djcyradm/login/',
                          {"username": "myuser@example.com",
                           "password": "cyrus"}
            ).status_code, 302)
        user = MailUsers.objects.get(
            is_active=True,
    ...
    def test_make_alias(self):
        ...
        self.assertTrue(user.check_password("cyrus"))
        self.client.post("/djcyradm/login/",
                         {"username": "myuser@example.com",
                          "password": "cyrus"
                          }
                         )
        ...
        self.assertFormError(
            ...
            'form',
            'dest',
            'No more aliases for this destination')
    ...
    def test_make_account(self):
        ...
        self.client.post(
            "/djcyradm/login/",
            {'username': "myuser@example.com", "password": "cyrus"})
        with self.settings(SYNCIMAP=False):

            resp = self.client.post(
                "/djcyradm/login/", {"username": "myuser@example.com",
                                     "password": "cyrus"
                                     }
                                 )
            self.assertEquals(resp.status_code, 302)
            ...
            loggedin = MailUsers.objects.get(
                is_active=True,
                is_main_cyrus_admin=False,
                username="myuser@example.com")
            ...
            if loggedin.groups.filter(name="domainadmins").exists():
                ...
                self.assertEquals(
                    resp.url,
                    "/djcyradm/mail-users/" + str(user.id) +
                    "/password/reset/")
                ...
                resp = self.client.post(
                    "/djcyradm/mail-users/add/",
                    {"username": "test", "domain": 2, "groups": 3,
                     "quota": "102400",
                     "max_aliases": domain.max_aliases_per_account})
